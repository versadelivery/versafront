...
interface GroupModalEditProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  editingGroup: string;
}

export default function GroupModalEdit({ isOpen, onOpenChange, editingGroup }: GroupModalEditProps) {
  const { updateCatalogGroup, isUpdatingGroup, catalogGroup, isLoadingGroup } = useCatalogGroup(editingGroup)
  ...
  const form = useForm({
    ...
    resolver: zodResolver(z.object({
      ...
      priority: z.coerce.number().min(1, 'A prioridade deve ser maior que 0'),
  ...
  useEffect(() => {
    if (catalogGroup) {
      form.setValue('name', catalogGroup.data.attributes.name)
      form.setValue('priority', catalogGroup.data.attributes.priority)
      form.setValue('description', catalogGroup.data.attributes.description)
      setPreviewImage(catalogGroup.data.attributes.image_url)
    }
  }, [catalogGroup])
  ...
  return (
    ...
    <DialogContent className='rounded-xs sm:h-auto max-w-[95vw] sm:max-w-[720px] p-4 sm:p-6 md:p-8 bg-white max-h-[90vh] overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-[#212121] [&::-webkit-scrollbar-thumb]:rounded-sm [&::-webkit-scrollbar]:px-2'>
      <DialogHeader>
        <DialogTitle className="text-start font-outfit text-xl md:text-2xl font-bold">EDITAR GRUPO</DialogTitle>
      </DialogHeader>
      {isLoadingGroup ? (
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            />
            <Button onClick={form.handleSubmit(onSubmit)} className="w-full" disabled={isUpdatingGroup}>{isUpdatingGroup ? <Loader2 className="animate-spin" /> : 'EDITAR GRUPO'}</Button>
          </form>
        </Form>
      )}
    </DialogContent>

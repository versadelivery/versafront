...
import { Camera, Loader2, Plus, Trash2, Egg, Copy } from "lucide-react";
...
export function EditItemModal({ id, isOpen, onOpenChange }: EditItemModalProps) {
  ...
  useEffect(() => {
    if (catalogItem && isOpen) {
      ...
      setAssemblyPricingMode(item.assembly_pricing_mode ?? 'sum');
      ...
      if (item.steps?.data?.length > 0) {
        ...
        setSteps(item.steps.data.map((step: any) => ({
          ...
          required: step.attributes.required ?? true,
          options: step.attributes.options?.data?.map((option: any) => ({
            id: option.id,
            name: option.attributes.name,
            price: option.attributes.price ? parseFloat(option.attributes.price).toFixed(2).replace('.', ',') : '',
          })) || [{ name: '', price: '' }],
        })));
      } else {
        setHasSteps(false);
  ...
  const handleDuplicateStep = (index: number) => {
    ...
    const duplicated: Step = {
      ...
      options: original.options.map((option) => ({ name: option.name, price: option.price })),
    };
  ...
  const proceedWithSubmit = async () => {
    ...
    try {
      ...
      formData.append('assembly_pricing_mode', hasSteps ? assemblyPricingMode : 'sum');
                ...
                <div key={step.id || stepIndex} className="bg-muted/30 rounded-lg p-3 space-y-2">
                  <div className="flex items-center gap-2">
                    <Input
                      ...
                      className="flex-1"
                    />
                    <Button type="button" variant="ghost" size="icon" className="h-9 w-9 shrink-0" onClick={() => handleDuplicateStep(stepIndex)} title="Duplicar etapa">
